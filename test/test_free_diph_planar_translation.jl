@testset "moving diphasic planar translation" begin
    q = 0.15
    λ1 = 1.0
    λ2 = 0.4
    J = 0.05
    ...
    @test abs(c1[2] + q / λ1) < 5e-8
    @test abs(c2[2] + q / λ2) < 8e-8
    ...
    @test maximum(abs.(p1γ[idxγ] .- p2γ[idxγ] .- J)) < 1e-8

    @test sol.step_diagnostics[end].max_normal_velocity_jump < 1e-9
end
